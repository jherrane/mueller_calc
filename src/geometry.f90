...
double precision, dimension(3,(mesh%M_ex+1)**3) :: nn
...
double precision, dimension(3) :: min_coord, max_coord
double precision :: delta, min_x, min_y, min_z, max_x
...
max_x = max_coord(1)
delta = abs(max_x - min_x)/(mesh%Nx-1)
...
subroutine build_grid2(mesh)
type (mesh_struct) :: mesh

double precision, dimension(:,:), allocatable :: nodes
double precision, dimension(3) :: min_coord, max_coord
double precision :: delta, min_x, min_y, min_z, max_x, max_y, max_z, PP(3,4), vol
...
!print*,'   Ave. tetra volume    =',real(vol/dble(mesh%N_tet))
...
double precision, dimension(3) :: cp
double precision, dimension(3,4) :: co
...
!function tetras_in_box(mesh,cube) result(tetras)
!  type (mesh_struct), intent(in) :: mesh
!  integer,intent(in) :: cube
!  integer, dimension(:), allocatable :: tetras

!  integer :: i1, las, tet(mesh%N_tet_cube)
!  las = 0
      
!  do i1 = 1,mesh%N_tet_cube
!     if (mesh%tetras(i1,cube) > 0) then 
!        tet(i1) = mesh%tetras(i1,cube)
!        las = las +1
!     end if
!  end do
